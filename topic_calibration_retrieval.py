...
for seed in range(1, 6):
    for candidate_size in [10, 20, 40, 60, 80, 100]:
      ...
      lstur_df = pd.read_json(f"data/recommendations_large/lstur_prediction_{candidate_size}_sample_{seed}.json", lines=True)
      nrms_df = pd.read_json(f"data/recommendations_large/nrms_prediction_{candidate_size}_sample_{seed}.json", lines=True)
      random_df = pd.read_json(f"data/recommendations_large/random_prediction_{candidate_size}_sample_{seed}.json", lines=True)
      pop_df = pd.read_json(f"data/recommendations_large/pop_prediction_{candidate_size}_sample_{seed}.json", lines=True)

      new_behaviors = pd.read_csv(f'data/extra_sampled_candidates/new_behaviors_{candidate_size}_sample_{seed}.tsv', delimiter='\t', header=None, names=['uid', 'date', 'history', 'candidates'])
      ...
      for progress_index, behavior in tqdm(behaviors.iterrows()):
          ...
          recommendations_collection['random'] = random_df[random_df['impr_index'] == behavior_id + 1]['pred_rank'].values.tolist()[0]
          recommendations_collection['pop'] = pop_df[pop_df['impr_index'] == behavior_id + 1]['pred_rank'].values.tolist()[0]
          recommendations_collection['lstur'] = lstur_df[lstur_df['impr_index'] == behavior_id + 1]['pred_rank'].values.tolist()[0]
          recommendations_collection['nrms'] = nrms_df[nrms_df['impr_index'] == behavior_id + 1]['pred_rank'].values.tolist()[0]
          ...
          results_df.to_csv(f'results/topic_calibration_{candidate_size}_sample_{seed}.csv')
